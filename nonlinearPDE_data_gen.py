...
def _set_zero_target_if_supported(env) -> None:
    """Keep legacy zero-target behavior only for envs without a target set."""
    if hasattr(env, "set_target_state") and getattr(env, "target_state", None) is None:
...
def _build_checkpoint_controller_if_needed(
    ...
    env_ctrl = gym.make(env_id, **_resolve_env_target_state(env_kwargs))
